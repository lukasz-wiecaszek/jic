...
\*===========================================================================*/
#define ISTREAM_BYTE_ORDER_BIG_ENDIAN        (0)
#define ISTREAM_BYTE_ORDER_LITTLE_ENDIAN      (1)

/** no error was encountered during stream handling */
#define ISTREAM_STATUS_OK                    (0)
/** end of stream was reached */
#define ISTREAM_STATUS_EOS_REACHED           (1 << 0)
/** an attempt to read/peak from not byte aligned stream was spotted */
#define ISTREAM_STATUS_IMPROPER_ALIGMENT     (1 << 1)
...
typedef std::function<void(const stream_position& before, const stream_position& after)> istream_error_function;
...
public:
    ...
    istream(const uint8_t* buffer, std::size_t size, const istream_error_function& function = nullptr) :
        m_buffer{buffer},
        m_size{size},
        m_position{},
        m_status{ISTREAM_STATUS_OK},
...
#pragma GCC diagnostic pop // restore previous diag context
     ...
     * and interprets them as T value.
     *
     ...
     * @param[out] value 'T' representation of retrieved bit pattern.
     *
     * @return true on success, false on failure.
     */
    template<typename T>
    bool read_bits(uint32_t number_of_bits, T& value)
    {
        ...
        if ((m_position.to_bits() + number_of_bits) <= (m_size * BITS_PER_BYTE)) {
            ...
            while (bits_remaining > 0) {
                /* there are more bits to be read than bits left in the stream's current byte */
                ...
                /* decrease number of remaining bits by the number of bits read in this iteration */
     ...
     */
    template<typename T>
    bool peek_bits(uint32_t number_of_bits, T& value)
    {
        static_assert(std::is_integral<T>::value, "T must be an integral type");
     ...
     *
     * code = (1U << leading_zero_bits) - 1 + read_bits(leading_zero_bits)
     *
     * where the value returned from read_bits(leading_zero_bits) is interpreted
    ...
    /**
     * Reads structure/tuple of data.
     *
     * @param[out] value 'structure<TYPES...>' representation of retrieved
     *                   (deserialized) bit pattern.
     *
     * @return true on success, false on failure.
     */
    template<typename... TYPES>
    bool read_structure(structure<TYPES...>& value)
    {
        const std::size_t size = sizeof...(TYPES);
        if (size == 0)
            return true;

        return structure_reader<size - 1, istream<STREAM_BYTE_ORDER>, structure<TYPES...>>{}(*this, value);
    }

    /**
     * Peaks structure/tuple of data.
     *
     * @param[out] value 'structure<TYPES...>' representation of retrieved
     *                   (deserialized) bit pattern.
     *
     ...
     */
    template<typename... TYPES>
    bool peak_structure(structure<TYPES...>& value)
    {
        std::size_t pos = tell();
        bool status = read_structure(value);
        seek(istream_seek_e::BEGINNING, pos);
     ...
     */
    void seek(istream_seek_e origin, long offset)
    {
        switch (origin) {
    ...
    static std::string status_to_string(uint32_t status)
    {
        if (status == ISTREAM_STATUS_OK)
            ...
            if (status & ISTREAM_STATUS_EOS_REACHED) {
                stream << "eos";
                if (status >= (ISTREAM_STATUS_EOS_REACHED << 1))
                    stream << ", ";
            }

            if (status & ISTREAM_STATUS_IMPROPER_ALIGMENT) {
                stream << "aligment";
                if (status >= (ISTREAM_STATUS_IMPROPER_ALIGMENT << 1))
                    stream << ", ";
            }

            if (status & ISTREAM_STATUS_STREAM_CORRUPTED) {
                stream << "corrupted";
                if (status >= (ISTREAM_STATUS_STREAM_CORRUPTED << 1))
                    stream << ", ";
            }
...
private:
   template<typename T>
   constexpr T to_cpu_byte_order(T x)
   {
       if constexpr (STREAM_BYTE_ORDER == ISTREAM_BYTE_ORDER_BIG_ENDIAN)
           return betocpu(x);
       else
       if constexpr (STREAM_BYTE_ORDER == ISTREAM_BYTE_ORDER_LITTLE_ENDIAN)
          return letocpu(x);
       else
           static_assert(
               (STREAM_BYTE_ORDER == ISTREAM_BYTE_ORDER_BIG_ENDIAN) ||
               (STREAM_BYTE_ORDER == ISTREAM_BYTE_ORDER_LITTLE_ENDIAN));
   }
    ...
    bool read(T& value)
    {
        if (unlikely(m_position.m_bit_position)) {
            m_status |= ISTREAM_STATUS_IMPROPER_ALIGMENT;
            if (m_error_function)
...
using istream_be = istream<ISTREAM_BYTE_ORDER_BIG_ENDIAN>;
using istream_le = istream<ISTREAM_BYTE_ORDER_LITTLE_ENDIAN>;
...
bool operator >> (STREAM& stream, uint8_t& value)
{
    return stream.template read_u8(value);
}

template<typename STREAM>
bool operator >> (STREAM& stream, int8_t& value)
{
    return stream.template read_s8(value);
}

template<typename STREAM>
bool operator >> (STREAM& stream, uint16_t& value)
{
    return stream.template read_u16(value);
}

template<typename STREAM>
bool operator >> (STREAM& stream, int16_t& value)
{
    return stream.template read_s16(value);
}

template<typename STREAM>
bool operator >> (STREAM& stream, uint32_t& value)
{
    return stream.template read_u32(value);
}

template<typename STREAM>
bool operator >> (STREAM& stream, int32_t& value)
{
    return stream.template read_s32(value);
}

template<typename STREAM>
bool operator >> (STREAM& stream, uint64_t& value)
{
    return stream.template read_u64(value);
}

template<typename STREAM>
bool operator >> (STREAM& stream, int64_t& value)
{
    return stream.template read_s64(value);
}
